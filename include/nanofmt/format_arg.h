...
namespace NANOFMT_NS::detail {
    template <typename T>
    struct value_type_map_impl {
        using type = T;
    };
    template <>
    struct value_type_map_impl<signed char> {
        using type = signed int;
    };
    template <>
    struct value_type_map_impl<unsigned char> {
        using type = signed int;
    };
    template <>
    struct value_type_map_impl<signed short> {
        using type = signed int;
    };
    template <>
    struct value_type_map_impl<unsigned short> {
        using type = signed int;
    };
    template <>
    struct value_type_map_impl<decltype(nullptr)> {
        using type = void const*;
    };
    template <typename T>
    using value_type_map_t = typename value_type_map_impl<std::decay_t<T>>::type;
    ...
    template <typename ValueT>
    constexpr format_value make_format_value(ValueT const& value) noexcept {
        if constexpr (std::is_constructible_v<format_value, detail::value_type_map_t<ValueT>>) {
            return detail::value_type_map_t<ValueT>(value);
        }
        else if constexpr (detail::has_formatter<ValueT>::value) {
            format_value::custom custom;
            custom.thunk = &detail::formatter_thunk<ValueT>;
            custom.value = detail::addressof(value);
            return custom;
        }
        else if constexpr (std::is_enum_v<ValueT>) {
            return static_cast<detail::value_type_map_t<std::underlying_type_t<ValueT>>>(value);
        }
    };
} // namespace NANOFMT_NS::detail
